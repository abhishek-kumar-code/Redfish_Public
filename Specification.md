...
* In the case of a request modifying a single property, if the property in the request can never be updated, such as when the property is read only, unknown or unsupported, an HTTP status code of [400](#status-000) shall be returned along with a representation of the resource containing a Message [annotation](#extended-information) specifying the non-updatable property. 
