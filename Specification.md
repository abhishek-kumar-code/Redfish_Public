...
* If a property in the request can never be specified and/or is inappropriate on a create, such as when a property is read only, a status code of [200](#status-200) shall be returned and a representation of the resource containing an [annotation](#extended-information) specifying the non-updatable property should be returned. In this success case, other properties are created as part of the resource.  Note that if an asynchronous task is started, the Asynchronous Operations rules shall be followed (thus a 200 would not be appropriate) and the rules in the above statement shall apply to the payload referenced in the location header.
