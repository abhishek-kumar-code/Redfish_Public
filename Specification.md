...
A service's implementation of the "EventService" resource may contain a property called "ServerSentEventUri".  If a client performs a GET on the URI specified by the "ServerSentEventUri", the service shall keep the connection open and conform to the [HTML5 Specification](#HTML5-Spec-SSE) until the client closes the socket.  Events generated by the service are sent to the client using the open connection.

The service shall use the "id" field in the SSE stream to uniquely indicate an event.  The value of the "id" field shall be the same as the "SequenceId" property in the event payload.  The value of the "SequenceId" property shall be a decimal value and shall be generated in a sequential manner.  This is to allow for a client to request for missed events if the connection is interrupted.

The service shall use the "event" field in the SSE stream to indicate the type of event being reported.  The value of the "event" field shall match one of the following forms:
* The "EventType" property of the Event payload in the data body
* An OEM definition matching the format "Oem.*CompanyName*", where *CompanyName* is the name of the company defining the event
...
The example payload below shows a stream containing a single event with the "id" field set to 1, "event" field set to Alert, and the "data" field containing a single Event object.
