...
The service shall use the "id" field in the SSE stream to uniquely indicate an event.  The value of the "id" field shall be the same as the "Id" property in the event payload.  The value of the "Id" property should be a decimal value, and should be generated in a sequential manner.  This is to allow for a client to request for missed events if the connection is interrupted.
