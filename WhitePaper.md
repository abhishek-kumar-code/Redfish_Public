...
Both the Schema and the metadata define which properties can be updated.  Properties that are marked "OData.Permissions/Read" or have read-only as true indicate these properties can never be updated.  But if they are marked writable, that doesn't necessarily indicate that a property can be written.  Instead an "OData.Permissions/ReadWrite" or read-only = false indicates that it might be writable.  The LongDescription (normative text) will indicate which properties are writable and under what conditions. 

Semantics on Redfish PUT/PATCH are such that attempting to update a non-writable property isn't treated as an error.  An implimentation of a Redfish Service will return a 200 with Extended Error Information indicating which properties were unable to be updated.
